...
def detect_slide_layout_tags(
    ...
    tags: set[str] = set()
    text_regions = [
        region for region in regions if region["kind"] == "shape" and region["type"] == "text"
    ]
...
def parse_theme_summary(theme_xml: str | None) -> dict[str, Any]:
    ...
    if text_styles_block:
        for match in re.finditer(
            r"<(title|headline|sub-headline|body|caption)\b([^>]*)/?>", text_styles_block.group(1)
        ):
            text_styles.append(
                compact_object(
                    {
...
def resolve_range_selection(entry: dict[str, Any], options: dict[str, Any]) -> dict[str, Any]:
    if options.get("label"):
        matched_range = next(
            (item for item in entry["ranges"] if item["label"] == options["label"]), None
        )
        ...
        if not query:
            ...
            for token in tokens:
                if token in search_text:
                    score += (
                        len(token) * 10 if re.search(r"[\u3400-\u9fff]", token) else len(token) * 6
                    )
